...
3. **Primary Method (AST):** If an Abstract Syntax Tree (AST) parser tool is available in your environment, use it to target the exact node signature and extract the full body block.
4. **Fallback Method (Buffer Extraction):** If no AST tool is available, do NOT attempt complex `sed` or `awk` bracket-matching. Use `grep -n` to find the anchor's starting line, then use `head` and `tail` to extract that line plus a generous 50-line buffer below it. Analyze the extracted block, and iterate with a larger buffer if the function is cut off.
...
## 🔄 State Hash  
[Exact output of md5sum/md5 -q. Example: e4d909c290d0fb1ca068ffaddf22cbd0]

## 🧬 Identity  
[2-3 sentences describing precise purpose and responsibility within the larger system architecture. What is this file's job?]

## 📍 Structural Map (Signature Anchors)
[Map out precise definition signatures so you can surgically AST/grep fetch them later. DO NOT USE LINE NUMBERS.]
...
## 🔗 Hard Dependencies  
* `[imported_module/path]` - [Why it is needed / what it provides]

## 🧠 Conceptual Usage & Flow  
[Brief explanation of how dependencies enable this file's identity. How does data flow in and out of this file?]

## ⚠️ State & Side Effects  
[Mutations to globals? Disk I/O? Database connections? Async effects? If clean: "Pure / Stateless"]
```

## 🧩 Architectural Flagging Protocol (For Files > 800 lines)
If a file exceeds 800 lines of code, do NOT attempt to fragment the Soul File. Generating multiple hash-dependent fragments creates infinite invalidation loops.
1. Generate a single Master Soul file following the standard template, focusing on the most critical Signature Anchors.
